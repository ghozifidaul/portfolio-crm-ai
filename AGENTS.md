...
Monorepo with two packages — both run with `bun`:

| Package | Stack | Dev server |
|---|---|---|
| `crm-ai-api/` | Bun + Hono + PostgreSQL (`pg`) | `:3000` |
| `crm-ai-ui/` | React 19 + Vite + Tailwind CSS v4 + React Router v7 | `:5173` (proxies `/api/*` → `:3000`) |

## Essential commands

```sh
# API setup (one-time)
cd crm-ai-api && bun install && createdb crm_ai && bun run setup

# Dev servers
cd crm-ai-api && bun run dev          # :3000
...
| `POST` | `/api/messages/direct` | JWT | Async — stores message, AI runs in background |
...
- PostgreSQL via `pg` Pool, connection from `DATABASE_URL` env var.
- Setup: `createdb crm_ai` → `bun run setup` (migrate + seed). Migration is idempotent (skips if tables exist).
...
- `entities` and `tags` are PostgreSQL `TEXT[]` arrays.
